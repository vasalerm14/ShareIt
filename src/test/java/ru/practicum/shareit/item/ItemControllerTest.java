...
class ItemControllerTest {
    ...
    void saveNewItem() throws Exception {
        when(itemService.saveNewItem(any(), anyInt())).thenReturn(itemDtoOut);
    ...
    void getItemById() throws Exception {
        when(itemService.getItemById(anyInt(), anyInt())).thenReturn(itemDtoOut);
    ...
    void getItemsByOwner() throws Exception {
        when(itemService.getItemsByOwner(anyInt(), anyInt(), anyInt())).thenReturn(List.of(itemDtoOut));
    ...
    void saveNewComment() throws Exception {
        final CommentDtoOut commentDtoOut = new CommentDtoOut(1, "comment", "user",
                LocalDateTime.now());

        when(itemService.saveNewComment(anyInt(), any(), anyInt())).thenReturn(commentDtoOut);
        ...
        mvc.perform(post("/items/1/comment")
                ...
                .andExpect(jsonPath("$.id", is(commentDtoOut.getId()), Integer.class))
